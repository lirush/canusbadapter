...
typedef struct
{
  ...
  struct
  {
    struct
    {
      ...
      uint32_t bytesToAck;        /* number of bytes sent to host in IN transaction but not acknowledged yet */
			uint8_t HaveNewData;				/*for OUT transaction*/
    }IO_Buffer;
    /* SETUP-transaction */
...
void USB_EP_SetNewDataOut(USB_EP_TypeDef EPx)
{
	USB_EPContext_TypeDef *ep = USB_EPContext + EPx;
	
	ep->Buffer.IO_Buffer.HaveNewData=SET;
	
	
}

uint8_t USB_EP_GetFlNewDataOut(USB_EP_TypeDef EPx)
{
	USB_EPContext_TypeDef *ep = USB_EPContext + EPx;
	
	return ep->Buffer.IO_Buffer.HaveNewData;
}

void USB_EP_ClearFlNewDataOut(USB_EP_TypeDef EPx)
{
	USB_EPContext_TypeDef *ep = USB_EPContext + EPx;
	
	ep->Buffer.IO_Buffer.HaveNewData=RESET;
}
        ...
        if (!(tmpCTRL & USB_SEP_CTRL_EPRDY) && ep->EP_WasScdone)
        {
          ...
          for (i = 0; i < count; i++)
          {
            ep->Buffer.IO_Buffer.pBuffer[ep->Buffer.IO_Buffer.offset + i] = EPx_RX_FIFO_DATA(EPx);
					...
					RingQuePut(&QueUSB,ep->Buffer.IO_Buffer.pBuffer,count-1);
...
/**
  * @brief  Checking USB Device in sleep
  *
  * @param  None
  *
  * @retval uint8_t (sleep or not)
  *
  */
extern uint8_t usb_dont_sleep;
static volatile uint32_t old_sfn_h=0;
void USB_DeviceCheckSleep(void)
{
	if(uiTimer(_2SEC))
	{
		if(old_sfn_h==USB_GetSFN())
			usb_dont_sleep= 0;
		old_sfn_h=USB_GetSFN();
	}
}
